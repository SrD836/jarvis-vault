...
8. **Indeed sin MCP** → MCP de Claude.ai está bloqueado por walled garden (solo responde a `client_id` de claude.ai). job-hunter agent usa SIEMPRE `WebFetch` público (`https://es.indeed.com/jobs?q=...&l=...`). El MCP `mcp__claude_ai_Indeed__*` queda solo documentado en `job-hunter/AGENTS.md`, NO usar.
9. **CV adapter LaTeX** → default Posquit0/Awesome-CV pinned en commit `964f99d` (último con FA5, compatible texlive 2022). Template Jinja2 en `vault/cv-templates/jarvis/resume.tex.j2` con sidebar+main 2 columnas. `xelatex` corre dentro del container gateway. Hard rule: Economía 2 años cursados sin completar, inglés B1 canon. Bullets de proyectos hardcoded en `hermes/cv_adapter_latex.py` (PROJECT_BULLETS dict).
10. **Telegram approval gate** → toda aplicación pasa por preview Telegram (PDF + inline URL buttons `apply/regen/skip`) antes de auto-apply. Endpoint `/api/trabajo/approval` valida HMAC-SHA256 + persiste `telegram_decision` en `applications.jsonl`. Anti-fraude rule sigue vigente: NO `applied:true` sin `linkedin_confirmation` real.
11. **Audit-before-add (anti-duplicación)** → ANTES de añadir cualquier feature/sección/endpoint nuevo, leer COMPLETO el archivo target + `git log --oneline -20 <file>` + grep de keywords relacionadas en el repo. Si la funcionalidad existe (aunque sea parcial) → EXTENDER lo existente, NUNCA crear sección paralela. Aplica especialmente a archivos largos donde la duplicación silenciosa es fácil (`Trabajo.jsx`, `server.js`, `main/AGENTS.md`, `cv_adapter_latex.py`). Violación = revert obligatorio + post-mortem en `memory/`. Caso de estudio: commit `f5ccbe8` jarvis-dashboard (Trabajo.jsx duplicó Preferencias + Certs).
