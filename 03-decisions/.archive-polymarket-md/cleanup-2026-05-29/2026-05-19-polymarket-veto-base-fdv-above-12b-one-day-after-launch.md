---
title: "Polymarket veto — base-fdv-above-12b-one-day-after-launch"
type: decision
date: 2026-05-19
decision: "Veto de tesis 'Base FDV above $12B one day after launch?' (rule P4_pre_event): pre-event slug + 591 d to resolution (>=7 threshold)"
alternatives:
  - "Aprobar tesis y entrar trade simulado"
  - "Vetar y mantener bankroll"
outcome: pending
outcome_observed_after_days: 30
tags: [decision, polymarket, bot, veto, p4_pre_event]
related:
  - "[[agents/polymarket-bot]]"
  - "[[agents/polymarket-bot/memory]]"
  - "[[projects/polymarket-veto-loop-bot]]"
---

# Veto: Base FDV above $12B one day after launch?

## Mercado

- **Slug**: `base-fdv-above-12b-one-day-after-launch`
- **Categoría**: uncategorized
- **Precio YES**: 0.3200
- **Volumen 24h**: 78659.61 USD
- **End date**: 2028-01-01T05:00:00Z

## Razón del veto

**Regla aplicada**: `P4_pre_event`

pre-event slug + 591 d to resolution (>=7 threshold)

## Human notes

_(no se toca por automatización)_
